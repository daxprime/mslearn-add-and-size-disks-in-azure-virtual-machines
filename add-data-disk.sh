...
sudo mkdir -p /srv/customers/coldstorage

# Attach the disk to the mount point.
sudo mount /dev/sdc1 /srv/customers/coldstorage

# Get the UUID of the new drive, /dev/sdc1, and save it as a variable.
UUID=$(sudo -i blkid | grep '/dev/sdc1' | cut -d"\"" -f2 )
...
echo "UUID=${UUID}    /srv/customers/coldstorage/    ext4    defaults,nofail    1    2" | sudo tee --append /etc/fstab
